...
def main():
    parser = argparse.ArgumentParser(description="Step 1: Crawl a BBS board index and save thread metadata to a CSV.")
    parser.add_argument("--board_id", type=int, default=config.BOARD_ID, help="The ID of the board to scrape.")
    parser.add_argument("--mode", type=str, default=config.RUN_MODE, choices=['overwrite', 'update'], help="Run mode: 'overwrite' or 'update'.")
    args = parser.parse_args()

    print(f"--- Running Step 1: Crawl Board Index for board {args.board_id} ---")

    # Initial page fetch just to get board name
    initial_url = urljoin(config.BASE_URL, f"thread.php?bid={args.board_id}&mode=topic&page=1")
    ...
    if board_name == "unknown":
        ...
        print("Please check your network settings and the board ID in config.py, then try again.")
    ...
    if args.mode == 'update':
        try:
            ...
            if csv_files:
                existing_csv_file = os.path.join(board_path, sorted(csv_files, reverse=True)[0])
            ...
            print(f"Could not determine last update date: {e}. Defaulting to overwrite.")
            args.mode = 'overwrite'
    ...
    fieldnames = ['id', 'title', 'author', 'post_date', 'replies', 'last_reply_date', 'last_reply_author', 'url']
    ...
    with open(new_csv_filepath, 'w', newline='', encoding='utf-8') as f:
        ...
        stop_crawling = False
        while True:
            index_url = urljoin(config.BASE_URL, f"thread.php?bid={args.board_id}&mode=topic&page={page_num}")
            ...
            for item in list_items:
                try:
                    ...
                    raw_last_reply_date = last_reply_div.select_one('.time').text.strip()
                    last_reply_date = normalize_date(raw_last_reply_date)

                    if args.mode == 'update' and last_update_time:
                        current_reply_dt = datetime.strptime(last_reply_date, '%Y-%m-%d')
                        if current_reply_dt.date() < last_update_time.date(): # Changed from <= to <
                            stop_crawling = True
                            break
                    ...
                    thread_info = {
                        ...
                        'post_date': post_date,
                        'replies': item.select_one('div.reply-num').text.strip() if item.select_one('div.reply-num') else '0',
                        'last_reply_author': last_reply_div.select_one('.name').text.strip(),
            ...
            print(f"Found and wrote {page_threads_found} new threads from page {page_num}.")
            if stop_crawling:
                print("Reached last update date. Stopping crawl.")
                break

            next_page_div = page_soup.select_one('div.paging-button:-soup-contains("下一页")')
    ...
    if args.mode == 'update' and existing_csv_file:
        ...
        all_threads = sorted(newly_crawled_threads.values(), key=lambda x: x['last_reply_date'], reverse=True)
